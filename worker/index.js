...
const DAILY_PROMPTS = [
  {
    slug: 'daily-kurdish-bride-cinematic',
    category: 'kurdish-style',
    title_ku: 'بووکی کوردی بە ستایلی سینەمایی',
    title_en: 'Cinematic Kurdish Bride',
    title_ar: 'عروس كردية سينمائية',
    description_ku: 'پرۆمپتی ڕۆژانە بۆ وێنەی بووکی کوردی.',
    prompt_text: 'Kurdish bride wearing traditional colorful Kurdish dress, elegant jewelry, mountain background, warm cinematic light, realistic photography, ultra detailed, professional portrait, 8K.',
    difficulty: 'easy',
    rating: 4.9,
    is_featured: 1,
    is_trending: 1,
    tags: ['kurdishbride', 'kurdish', 'portrait']
  },
  {
    slug: 'daily-ramadan-mosque-glow',
    category: 'islamic',
    title_ku: 'مزگەوتی ڕەمەزان بە ڕۆشنایی گەرم',
    title_en: 'Ramadan Mosque Glow',
    title_ar: 'مسجد رمضان بإضاءة دافئة',
    description_ku: 'پرۆمپتی ڕۆژانە بۆ دیمەنی ئیسلامی.',
    prompt_text: 'Beautiful mosque during Ramadan night, glowing lanterns, crescent moon, peaceful atmosphere, warm cinematic lighting, Islamic architecture, realistic wallpaper style, high quality.',
    difficulty: 'easy',
    rating: 4.9,
    is_featured: 0,
    is_trending: 1,
    tags: ['ramadan', 'islamic', 'mosque']
  },
  {
    ...
    is_featured: 1,
    is_trending: 1,
    tags: ['couple', 'hug', 'twophotos']
  },
  {
    slug: 'daily-luxury-car-night-ad',
    category: 'cars',
    title_ku: 'ئۆتۆمبێلی لاکژەری لە شەودا',
    title_en: 'Luxury Car Night Ad',
    title_ar: 'سيارة فاخرة ليلاً',
    description_ku: 'پرۆمپتی ڕۆژانە بۆ کار و ڕیکلام.',
    prompt_text: 'Luxury car photography at night, neon street reflections, cinematic rain, sharp details, premium advertising look, dramatic lighting, ultra realistic, high quality.',
    difficulty: 'medium',
    rating: 4.8,
    is_featured: 0,
    is_trending: 1,
    tags: ['luxurycar', 'cars', 'cinematic']
  }
];
...
async function publishDailyPrompt(env) {
  ...
  const result = await env.DB.prepare(
    'INSERT INTO prompts (slug, category_id, title_ku, title_en, title_ar, description_ku, prompt_text, difficulty, rating, is_featured, is_trending, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)'
  ).bind(slug, category.id, template.title_ku, template.title_en, template.title_ar, template.description_ku, template.prompt_text, template.difficulty, template.rating, template.is_featured, template.is_trending).run();
...
async function createPromptFromRequest(request, env) {
  ...
  const result = await env.DB.prepare(
    'INSERT INTO prompts (slug, category_id, title_ku, title_en, title_ar, description_ku, description_en, description_ar, prompt_text, negative_prompt, preview_image_url, difficulty, rating, is_featured, is_trending, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)'
  ).bind(slug, category.id, body.title_ku, body.title_en || null, body.title_ar || null, body.description_ku || null, body.description_en || null, body.description_ar || null, body.prompt_text, body.negative_prompt || null, body.preview_image_url || null, body.difficulty || 'easy', body.rating || 4.8, body.is_featured ? 1 : 0, body.is_trending ? 1 : 0).run();
...
async function attachTags(env, promptId, tags) {
  for (const rawTag of tags) {
    ...
    const name = rawTag.startsWith('#') ? rawTag : `#${rawTag}`;
