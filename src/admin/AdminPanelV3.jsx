...
import { BarChart3, CalendarClock, Edit3, KeyRound, Plus, RefreshCw, Search, ShieldCheck, Sparkles, Trash2, X } from 'lucide-react';

const API_BASE = 'https://promptstan-api.hhhh46529.workers.dev';
const emptyPrompt = { title_ku: '', title_en: '', title_ar: '', description_ku: '', prompt_text: '', category_slug: 'kurdish-style', tags: 'person,edit', difficulty: 'easy', rating: 4.8, is_featured: false, is_trending: true };
const pageSize = 10;
...
export default function AdminPanelV3() {
  ...
  return <main className="adminShell" dir="rtl">
    ...
    <section className="adminCard"><div className="adminCardTitle"><Search size={22} /><h2>پرۆمپتە بڵاوکراوەکان</h2></div><div className="adminSearch"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search prompts..." /><span>{filteredPrompts.length} results</span></div><div className="adminPromptList">{shownPrompts.length === 0 ? <p>هیچ پرۆمپتێک نەدۆزرایەوە.</p> : shownPrompts.map((prompt) => <article key={prompt.id} className="adminPromptItem"><div><strong>{prompt.title_ku || prompt.title_en}</strong><small>{prompt.category_name} • 👁 {prompt.views || 0} • 📋 {prompt.copies || 0}</small></div><span>{prompt.is_trending ? '🔥 Trending' : 'Prompt'}</span><div className="adminPromptActions"><button onClick={() => startEdit(prompt)} disabled={!savedToken || loading}><Edit3 size={16} /> Edit</button><button className="dangerButton" onClick={() => removePrompt(prompt)} disabled={!savedToken || loading}><Trash2 size={16} /> Remove</button></div></article>)}</div><div className="pagination"><button disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>Prev</button><strong>{page} / {pageCount}</strong><button disabled={page >= pageCount} onClick={() => setPage((p) => p + 1)}>Next</button></div></section>
  </main>;
}
