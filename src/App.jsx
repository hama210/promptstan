...
const ui = {
  KU: {
    ...
    hero: 'شوێنی هەموو پرۆمپتەکانی AI',
    desc: 'پرۆمپتی ئامادە بۆ دەستکاریکردنی وێنە، پۆرترێت، باکگراوند، بەرهەم و ستایلی سینەمایی.',
    searchPlaceholder: 'چی دەتەوێت دروست بکەیت؟',
    search: 'گەڕان',
    today: 'پرۆمپتی ئەمڕۆ',
    todayTitle: 'پۆرترێتی سینەمایی بە ڕۆشنایی زێڕین',
    todayDesc: 'یەک پرۆمپتی پڕۆفیشناڵ بۆ دروستکردنی وێنەی سینەمایی، ڕوون، جوان و ئامادە بۆ سۆشیال میدیا.',
    copy: 'کۆپی',
...
export default function App() {
  ...
  const favoritePrompts = useMemo(() => promptItems.filter((item) => favoriteIds.includes(item.id)), [favoriteIds]);
    ...
    return (
      <article className="promptCard">
        ...
        <div className="cardActions">
          <button onClick={() => copyText(item.text, item.title)} className="copyButton"><Copy size={18} /> {t.copy}</button>
          <button className="previewButton" onClick={() => setActivePrompt(item)}><Eye size={18} /> {t.preview}</button>
        </div>
      </article>
    );
  };
  ...
  return (
    <main className={isLtr ? 'app ltr' : 'app'} dir={isLtr ? 'ltr' : 'rtl'}>
      ...
      {activePrompt && (
        <div className="modalOverlay" onClick={() => setActivePrompt(null)}>
          <section className="promptModal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className={`modalVisual ${activePrompt.gradient}`}><span>{activePrompt.badge}</span><strong>{activePrompt.imageTitle}</strong></div>
            <div className="modalContent">
              ...
              <div className="promptBox"><div className="promptBoxHeader"><strong>{t.prompt}</strong><button onClick={() => copyText(activePrompt.text, activePrompt.title)}><Copy size={16} /> {t.copy}</button></div><p>{activePrompt.text}</p></div>
              <div className="tagList">{activePrompt.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div>
      ...
      <nav className="navbar">
        <div className="brand"><div className="logoMark"><Wand2 size={22} /></div><div><strong>پڕۆمپتستان</strong><span>Promptstan</span></div></div>
        <div className="navActions"><div className="languageTabs">{languages.map((item) => <button key={item.code} className={language === item.code ? 'active' : ''} onClick={() => setLanguage(item.code)}>{item.flag} {item.code}</button>)}</div><button className="ghost" onClick={() => scrollToSection('favorites')}><Heart size={18} /> {favoriteIds.length}</button></div>
      </nav>
