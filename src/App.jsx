...
import { loadLibraryData, searchLibrary, trackPromptAction } from './services/apiLibrary.js';
...
export default function App() {
  ...
  useEffect(() => {
    loadLibraryData().then(setLibrary);
  }, []);
  ...
  const PromptCard = ({ item }) => {
    ...
    return (
      <article className="promptCard">
        ...
        <div className={`promptImage ${item.gradient || 'purple'}`}><span className="promptBadge">{item.badge}</span><strong>{item.imageTitle}</strong></div>
        ...
        <div className="cardActions"><button onClick={() => copyText(item.text, item.title, item.id)} className="copyButton"><Copy size={18} /> {t.copy}</button><button className="previewButton" onClick={() => openPrompt(item)}><Eye size={18} /> {t.preview}</button></div>
      </article>
    );
  };
